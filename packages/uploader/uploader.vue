<template>
    <div class="upload-table" v-loading.body="isUploading" element-loading-text="正在上传中，请稍等......">
        <section class="nv-layout-form search-form" slot="top">
          <el-form ref="form">
            <nv-layout-form>
              <el-button @click="search();" slot="search-button" type="primary" icon="el-icon-search">搜索</el-button>
              <template slot="search-item">
                  <el-form-item label="关键字" class="search-form-one">
                    <el-input v-model="fileName" placeholder="请输入关键字" size="middle" @keyup.enter.native = "search"></el-input>
                  </el-form-item>
              </template>
              <template slot="extra-item">
                    <el-button @click="createdNewFolder" type="primary" size="mini" :disabled ="isSearch" v-show="isShowCreate">
                        新建文件夹
                    </el-button>
                    <el-button @click="downloadChooseRows" type="primary" size="mini" v-show="isShowDownload">
                        下载
                    </el-button>
                    <el-button @click="deleteSelectedAndChildren" type="primary" size="mini" v-show="isShowDelete">
                        删除
                    </el-button>
                    <el-button   @click="batchHandOnToDialog()" size="mini" v-show="levelList.length" type="primary">
                        移交
                    </el-button>
                    ...
                    <el-button @click="refresh" type="primary" size="mini"  style="margin-left:10px;" >
                        刷新
                    </el-button>
        ...
        <div class="uploader-top bg-primary">
            <div class="selector">
              <el-checkbox v-model="isAllSelected" :disabled="!list.length" style="color:#fff;">全选</el-checkbox>
            </div>
            <el-breadcrumb class="nav nv-link" separator=" > ">
                <el-breadcrumb-item v-for="(item,index)  in levelList" :key="item.id">
                    <span v-if='index==levelList.length-1' style="color:#fff;">{{item.name}}</span>
                    <span v-else @click="setParentCode(item.id, item.name, 'delete', item)" style="color:#fff;">{{item.name}}</span>
                </el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        ...
        <div class="home-detail__page" v-if="totalCount > 10">
            <el-pagination
...
export default {
  ...
  data() {
    return {
      ...
      isBatch: null,
      // listFileStatus:null,
      // uploadURL: "api/fileWithftp/upload/project/type",//`${process.env.BASE_URL || '/api'}/file/upload/project/type`,
      uploadURL: "api/fileWithftp/upload/project/type",
      fileData: {},
  ...
  mounted() {
    this.uploadHeaders = {
      Authorization: nenv.storage.userStorage.token // localStorage.getItem("user.token")
    };
    this.fileData = {
  ...
  methods: {
      ...
      getOperatePermission()
        .then(({ data = {} }) => {
          ...
          list.forEach(item => {
            ...
            } else if ("RENAME" === item.OPERATE_NAME) {
    ...
    batchHandOnToDialog() {
      ...
      if (self.selectedFiles.length === 0) {
        return this.$message.info("请勾选要移交的文件");
      }
      self.selectedFiles.forEach(item => {
        ...
        if (self.userName !== item.CREATE_USER) {
      ...
      if (type == "NO") {
        ...
        FileView(req)
          .then(({ data }) => {
            ...
            // 2018-07-02 浏览器自带预览pdf就行 不需要这些
            // const filePath = data.filePath;
            // if(filePath.substring(filePath.length-3)=='pdf'){
            //   url = "/decision/pdfjs/web/viewer.html?file=../../../decision/file/" + data.filePath;
            // }
    ...
    refresh () {
      ...
      this.fetchData(levelList[levelList.length-1].id)
    },
    modifyComplete(row, newName) {
      let me = this;
      me.rename = newName;
      if (me.rename.length > 50) {
        me.$message.error("修改文件夹名称长度不能大于50");
        return;
      }
      if (row.IS_DIRECTORY === "YES") {
        //文件夹重命名
    ...
    deleteSelectedAndChildren() {
      let me = this;
      let rows = me.selectedFiles;
      if (rows.length === 0) {
        me.$message.info("请勾选要删除的文件或文件夹");
        return;
      }
      let canDelete = true;
      me.selectedFiles.forEach(item => {
        if (!item.permission.delete) {
          me.$message.info("所选文件无权删除！");
          canDelete = false;
          return;
        }
      });
